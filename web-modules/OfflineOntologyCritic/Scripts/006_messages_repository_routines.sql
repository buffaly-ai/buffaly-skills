...
-- Return-table signatures cannot be changed with CREATE OR REPLACE. Drop this small
-- dependency family explicitly so an existing 16-column install can append TurnRowID last.
DROP FUNCTION IF EXISTS get_message_sp(integer);
DROP FUNCTION IF EXISTS get_messages_sp();
DROP FUNCTION IF EXISTS get_messages_by_session_id_sp(integer);
DROP FUNCTION IF EXISTS get_message_rows();
...
CREATE INDEX IF NOT EXISTS ix_messages_session_id_compaction_epoch_key_message_id ON messages(session_id, compaction_epoch_key, message_id);

-- Keep the shared result projection as an inlineable view. Filtering a set-returning
-- SQL function forces PostgreSQL to materialize every message before applying predicates.
CREATE OR REPLACE VIEW message_rows AS
	SELECT message_id AS "MessageID",session_id AS "SessionID",sequence_number AS "SequenceNumber",role AS "Role",content AS "Content",tool_name AS "ToolName",tool_arguments AS "ToolArguments",call_id AS "CallID",date_created AS "DateCreated",last_updated AS "LastUpdated",data AS "Data",is_compacted AS "IsCompacted",compaction_epoch AS "CompactionEpoch",message_key AS "MessageKey",turn_id AS "TurnID",compaction_epoch_key AS "CompactionEpochKey",turn_row_id AS "TurnRowID"
	FROM messages;
...
LANGUAGE sql AS $$
	SELECT message_id,session_id,sequence_number,role,content,tool_name,tool_arguments,call_id,date_created,last_updated,data,is_compacted,compaction_epoch,message_key,turn_id,compaction_epoch_key,turn_row_id
	FROM messages;
$$;
...
CREATE OR REPLACE FUNCTION copy_message_sp(p_message_id integer) RETURNS TABLE ("MessageID" integer) LANGUAGE sql AS $$
	...
	SELECT session_id,sequence_number,role,content,tool_name,tool_arguments,call_id,now(),now(),data,is_compacted,compaction_epoch,message_key || '-copy-' || replace(clock_timestamp()::text, ' ', '-') || '-' || floor(random() * 1000000)::integer::text,turn_id,turn_row_id,compaction_epoch_key FROM messages WHERE message_id=p_message_id
	RETURNING message_id;
$$;
