...
Source actions and intermediate collector envelopes can exceed the runtime's inline-string threshold. Every potentially large return crosses the wrapper as `StringRef` and is explicitly materialized through `ToResolveStringReference` before JSON parsing or analysis. This applies to Web Properties responses, Google Ads reports, LinkedIn analytics, OpenAI usage, Reddit reports, collector envelopes, and model responses; otherwise handle text can be mistaken for JSON or an analyzer can lose the collector envelope.
