...
The recursive GA4 and Tracking Logs fan-out helpers accept their property and accumulated-response arguments through the normal `string` contract and materialize them at every method entry. This preserves ordinary inline source JSON while also resolving any value ProtoScript spools between recursive calls. Declaring these helper parameters as `StringRef` is incorrect because inline property JSON does not bind through that contract reliably; omitting materialization is also incorrect because a spooled value is otherwise parsed as its handle token.
