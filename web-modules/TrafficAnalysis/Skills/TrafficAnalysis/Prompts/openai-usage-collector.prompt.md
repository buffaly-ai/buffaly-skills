...
Immediately before saving, read this prompt as raw UTF-8 and compute lowercase SHA-256 over the exact bytes. Save `data-stream/openai-usage.json` with the common camelCase envelope and exact `promptProvenance`. Save a failed envelope on errors without affecting other collectors.
