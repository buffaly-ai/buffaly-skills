...
6. Save `artifacts/data-stream/traffic-properties.json` in the current session with this exact camelCase envelope: `source`, `collectedAt`, `timeframe`, `status`, `rawData`, `analysis`, `promptProvenance`. Provenance contains `promptFile`, `promptHash` prefixed `sha256:`, and full `promptText`.
