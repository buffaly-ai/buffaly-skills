...
function addScript(src) { return new Promise((resolve, reject) => { const existing = [...document.scripts].find(x => x.src === new URL(src, location.href).href); if (existing) { if (window.CodeMirror) resolve(); else existing.addEventListener("load", resolve, { once: true }); return; } const script = document.createElement("script"); script.src = src; script.onload = resolve; script.onerror = () => reject(new Error("Could not load Source Viewer asset: " + src)); document.head.appendChild(script); }); }
async function loadAssets(language) { if (!assetsPromise) { addStyle(BASE + "vendor/codemirror/lib/codemirror.css"); assetsPromise = addScript(BASE + "vendor/codemirror/lib/codemirror.js"); } await assetsPromise; if (language === "javascript") await addScript(BASE + "vendor/codemirror/mode/javascript/javascript.js"); if (language === "csharp") await addScript(BASE + "vendor/codemirror/mode/clike/clike.js"); }
...
class SourceViewerModule extends HTMLElement {
  ...
  configure(configuration) {
    ...
    if (!Object.prototype.hasOwnProperty.call(LANGUAGES, state.language)) throw new Error("Unsupported source language.");
