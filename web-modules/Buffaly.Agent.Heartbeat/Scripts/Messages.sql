...
CREATE UNIQUE NONCLUSTERED INDEX [IX_Messages_MessageKey] ON [dbo].[Messages]
(
	[MessageKey] ASC
)
...
CREATE NONCLUSTERED INDEX [IX_Messages_TurnRow] ON [dbo].[Messages]
(
	[SessionID] ASC,
	[TurnRowID] ASC,
	[DateCreated] DESC,
...
CREATE NONCLUSTERED INDEX [IX_Messages_UnindexedTurn] ON [dbo].[Messages]
(
	[SessionID] ASC,
	[MessageID] ASC
)
INCLUDE ([TurnID], [Role], [DateCreated])
WHERE [TurnRowID] IS NULL AND [TurnID] IS NOT NULL
WITH (STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
GO
