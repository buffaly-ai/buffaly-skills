...
CREATE OR REPLACE FUNCTION messages_get_lifecycle_by_session_idturn_keys_sp(p_session_id integer, p_turn_keys_csv text) RETURNS TABLE ("MessageID" integer,"SessionID" integer,"SequenceNumber" integer,"Role" text,"Content" text,"ToolName" text,"ToolArguments" text,"CallID" text,"DateCreated" timestamp,"LastUpdated" timestamp,"Data" text,"IsCompacted" boolean,"CompactionEpoch" integer,"MessageKey" text,"TurnID" text,"CompactionEpochKey" text) LANGUAGE sql AS $$ SELECT r.* FROM get_message_rows() r JOIN regexp_split_to_table(COALESCE(p_turn_keys_csv,''), ',') keys(turn_key) ON r."TurnID"=trim(keys.turn_key) WHERE r."SessionID"=p_session_id AND r."Role"='Lifecycle' ORDER BY r."DateCreated" ASC, r."MessageID" ASC; $$;
