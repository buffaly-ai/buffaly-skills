...
BEGIN
	...
	(
		...
		WHERE session_row.is_archived = false
			...
			(
				...
				OR
				(
					COALESCE(session_row.agent_name, '') NOT IN ('level-2', 'online-session-memory-critic', 'online-action-critic', 'code-review-agent', 'code-review-agent-v3')
					AND COALESCE(session_row.data::jsonb ->> 'SessionKind', '') <> 'CodexSubAgent'
					AND session_row.session_key NOT LIKE '%-online-memory-critic'
					AND session_row.session_key NOT LIKE '%-online-action-critic'
					AND session_row.session_key NOT LIKE '%.CodeReviewAgentV3'
				)
			)
	...
	SELECT eligible.session_id, eligible.session_key, eligible.parent_session_id, eligible.parent_session_key,
		eligible.session_name, eligible.agent_name, eligible.project_name, eligible.project_file_path,
		eligible.provider, eligible.model_name, eligible.reasoning_level, eligible.prompt_context,
		eligible.compaction_provider, eligible.data, eligible.date_created, eligible.last_updated, eligible.last_updated,
		NULL::integer, NULL::integer, NULL::integer
	FROM eligible_rows eligible
	WHERE eligible.row_ordinal BETWEEN p_skip_rows + 1 AND p_skip_rows + p_num_rows + 1
	ORDER BY eligible.row_ordinal;
END;
$$;
...
BEGIN
	...
	SELECT row.session_id, row.session_key, row.parent_session_id, parent.session_key, row.session_name,
		...
		row.prompt_context, row.compaction_provider, row.data, row.date_created, row.last_updated,
...
DROP FUNCTION IF EXISTS "Sessions_GetSidebarSelectedBranchSp"(varchar);
CREATE FUNCTION "Sessions_GetSidebarSelectedBranchSp"(p_session_key varchar)
RETURNS TABLE (
	"SessionID" integer,"SessionKey" text,"ParentSessionID" integer,"ParentSessionKey" text,
	"SessionName" text,"AgentName" text,"ProjectName" text,"ProjectFilePath" text,"Provider" text,
	"ModelName" text,"ReasoningLevel" text,"PromptContext" text,"CompactionProvider" text,"Data" text,
	"DateCreated" timestamp,"OwnLastUpdated" timestamp,"EffectiveLastUpdated" timestamp,
	"RootSessionID" integer,"RootOrdinal" integer,"HierarchyDepth" integer)
LANGUAGE sql AS $$ SELECT * FROM sessions_get_sidebar_selected_branch_sp(p_session_key::text); $$;

SELECT record_schema_migration('013_session_sidebar_all_recent_contracts');
