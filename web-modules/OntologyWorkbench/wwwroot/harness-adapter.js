(function () {
  ...
  window.ProviderCatalogService = {
    GetProviderCatalogObjectAsync: async request => {
      const response = await fetch('/harness/api/provider-catalog', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(request || {})
      });
      const text = await response.text();
      if (!response.ok) throw new Error(text || ('Provider catalog request failed: ' + response.status));
