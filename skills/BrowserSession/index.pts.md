...
- Changed `ToFillBrowserSelectorWithSecret` and `ToFillBrowserPasswordWithSecret` to declare `secretValue` as `StringRef` so `ToGetUserSecret` handles materialize before reaching the C# browser secret-fill helper; primitive `string` preserves opaque handles and triggers the fail-closed guard.
