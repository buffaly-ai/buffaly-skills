...
- Added `ToOpenManagedCdpBrowserSession(url, browserKey="")` as the generic managed-browser route. It delegates atomically to the C# coordinator, uses the configured default key when omitted, and accepts no caller-controlled launcher, profile, executable, or port.
