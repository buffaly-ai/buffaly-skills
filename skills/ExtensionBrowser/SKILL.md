...
- `extension/release/Buffaly-Browser-Agent-0.2.41-chrome.zip`: Chrome Web Store upload artifact produced by `npm run release:check`.
...
The generated archive must match version 0.2.41 and pass `scripts/verify-release.mjs` before staging the indexed release artifact. The conversation iframe explicitly delegates `microphone` to the selected Buffaly origin; Chrome then applies the embedded Buffaly page origin's normal microphone site permission.
