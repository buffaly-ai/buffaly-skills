...
assert.match(background, /async function ensureVisibleConversation[\s\S]{0,900}conversationForContext\(server, browserContextId\) \|\| conversationsForServer\(server\)\[0\][\s\S]{0,500}createDurableConversation/, 'startup must reuse a valid context, active, or first authoritative conversation and create only when none exists');
assert.doesNotMatch(panel, /url\.searchParams\.set\('installationRegistrationId'/, 'side-panel iframe navigation must not restore the obsolete unauthenticated installation catalog gate');
assert.doesNotMatch(background, /url\.searchParams\.set\('installationRegistrationId'/, 'pop-out iframe navigation must not restore the obsolete unauthenticated installation catalog gate');
