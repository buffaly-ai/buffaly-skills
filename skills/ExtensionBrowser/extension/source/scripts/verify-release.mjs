...
check(sidepanelSource.includes("url.searchParams.set('sessionKey', bootstrap.SessionKey)") && !sidepanelSource.includes('navigationToken'), 'side panel iframe must use durable sessionKey navigation without one-time tokens');
...
check(backgroundSource.includes("url.searchParams.set('sessionKey', prepared.SessionKey)") && !backgroundSource.includes("url.searchParams.set('navigationToken'"), 'pop-out must use durable sessionKey navigation without one-time tokens');
...
check(!fs.readFileSync(path.join(root, 'lib/buffaly-connection.ts'), 'utf8').includes("/web-modules/ExtensionBrowser/api/conversations/navigation-token"), 'normal extension client flow must not request navigation tokens');
check(!fs.readFileSync(path.join(root, 'lib/buffaly-connection.ts'), 'utf8').includes('issueLegacyNavigationToken') && backgroundSource.includes('async function ensureDurableConversation') && backgroundSource.includes("url.searchParams.set('sessionKey', prepared.SessionKey)") && backgroundSource.includes("url.searchParams.set('installationRegistrationId', prepared.InstallationRegistrationId)"), 'pop-out must normalize legacy records to durable SessionKey records before opening the installation-authorized session URL');
...
check(sidepanelSource.includes('`session:${conversation.SessionKey}`') && sidepanelSource.includes('`legacy:${conversation.SessionBindingId}`'), 'sidepanel selection values must distinguish durable sessions from legacy binding ids');
check(sidepanelSource.includes("url.searchParams.set('installationRegistrationId', bootstrap.InstallationRegistrationId)"), 'sidepanel conversation URL must carry the installation catalog owner for reload-safe bootstrap authorization');
