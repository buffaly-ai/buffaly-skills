...
for (const permission of ['sidePanel', 'storage', 'tabs', 'scripting', 'activeTab', 'debugger']) {
  check(manifest.permissions.includes(permission), `required permission is missing: ${permission}`);
}
check(sidepanel.includes('BuffalyEmbedSettings'), 'side panel embed settings storage contract is missing');
