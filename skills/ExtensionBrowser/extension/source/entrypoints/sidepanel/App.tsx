...
export default function App() {
  ...
  const recoverConversations = useCallback(async () => {
    setBusy('recover-conversations'); setError('');
    try {
      const recovered = await chrome.runtime.sendMessage({ type: 'recover_buffaly_conversations' }) as WorkerResponse<{ Recovered: number }> | undefined;
      if (!recovered?.ok) throw new Error(recovered?.error || 'Recovered conversations could not be loaded.');
      await refreshServers();
      setError(`Recovered conversations loaded: ${recovered.data?.Recovered ?? 0}`);
    } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setBusy(''); }
  }, [refreshServers]);
