# Buffaly Browser Agent 0.2.48 / WebModule 0.1.31 validation

0.2.48 / WebModule 0.1.31 replace the ineffective content-settings microphone recovery with a package-owned top-level setup page on the selected Buffaly server. An explicit user click invokes the real server-origin `getUserMedia({audio:true})`, stops the test track immediately, and reports the exact browser result. The side panel opens this page without receiving extension-level audio authority and preserves exact embedded-composer exceptions for diagnosis.
