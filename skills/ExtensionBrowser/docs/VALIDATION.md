# Buffaly Browser Agent 0.2.12 validation

0.2.12 keeps reusable installation authorization in the MV3 service worker, keeps the package-owned channel alive with typed heartbeats, exposes navigation-specific bound actions, and navigates the side-panel iframe with only a single-use conversation token.
