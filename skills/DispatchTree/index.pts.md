...
- `ToDispatchPlannedTask` is the only model-facing delivery action. It verifies the selected prototype is beneath `DispatchMemoryRoot` and requires its assigned `SessionKey` to exactly equal the requested destination before calling the typed session queue service. Generic direct send remains outside the Dispatch action root.
