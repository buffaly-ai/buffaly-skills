...
- `ToDispatchPlannedTask` is the only model-facing delivery action. It verifies the selected prototype is beneath `DispatchMemoryRoot` and resolves the destination exclusively from that leaf's non-empty assigned `SessionKey` before calling the typed session queue service. The caller cannot supply or override a destination, and generic direct send remains outside the Dispatch action root.
