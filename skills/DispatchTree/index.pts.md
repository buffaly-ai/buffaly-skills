...
- `ToGetDispatchNode` accepts one exact prototype selected from the tree, recursively verifies it at any depth beneath the current runtime's `DispatchMemoryRoot`, and returns a shallow read-only projection containing its assigned properties, including `SessionKey`. The recursion is required because `GetDescendants()` returns immediate children rather than the complete transitive hierarchy.
