...
- Each bounded action directly inherits `DispatchTreeSkillAction` and all three dispatch-agent context roots: `DispatchContext`, `ErrorLogHandlerContextPrompt`, and `EmailSummaryHandlerContextPrompt`. The host registers only immediate descendants of a supplemental PromptContext action root, so direct inheritance makes the same bounded tree surface callable from the first turn in each scenario while preserving the public Skill discovery surface. Do not insert an intermediate context-action prototype.
