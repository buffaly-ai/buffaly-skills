...
AS
	...
	(
		...
		WHERE	root.ParentSessionID IS NULL
				...
				AND
				(
					root.SessionKey = N'Buffaly.CodeReviews.Global'
					OR
					(
						ISNULL(root.AgentName, N'') NOT IN (N'code-review-agent', N'code-review-agent-v3')
						AND root.SessionKey NOT LIKE N'%.CodeReviewAgentV3'
					)
				)
